...
public class AccelListener extends CordovaPlugin implements SensorEventListener {
    ...
    */
    private int start() {
        ...
        List<Sensor> list = this.sensorManager.getSensorList(Sensor.TYPE_LINEAR_ACCELERATION);//TYPE_ACCELEROMETER

        // If found, then register as listener
        if ((list != null) && (list.size() > 0)) {
          this.mSensor = list.get(0);
          if (this.sensorManager.registerListener(this, this.mSensor, SensorManager.SENSOR_DELAY_FASTEST)) {//SENSOR_DELAY_UI
              this.setStatus(AccelListener.STARTING);
              // CB-11531: Mark accuracy as 'reliable' - this is complementary to
              // setting it to 'unreliable' 'stop' method
              this.accuracy = SensorManager.SENSOR_STATUS_ACCURACY_HIGH;//SENSOR_STATUS_ACCURACY_MEDIUM
          } else {
